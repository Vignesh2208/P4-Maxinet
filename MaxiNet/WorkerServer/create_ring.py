...
all_links = []
if nswitches == 2 :
	switches = range(0, nswitches-1)
else:
	switches = range(0, nswitches)
for i in switches:
	all_links.append(["s" + str(i + 1), "s" + str(((i+1) % nswitches) + 1), link_delay])
...
with open('ring_topo.json', 'w') as f:
